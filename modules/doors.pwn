...
* Niedokoñczona komenda /drzwi I TYLKO TYLE*/
...
CallBack:OnPlayerEnterDoors(playerid, uid)
{
	if(!DoorCache[uid][dLock])
	{
		if(!Player[playerid][Admin])
		{
		    if(GetPlayerMoney(playerid) >= DoorCache[uid][dFee])
		    {
			    ...
			    return DInfo(playerid, "Nie staæ Ciê na wejœcie do œrodka.");
		}
		else
		{
	    	SetPlayerPos( playerid, DoorCache[uid][dExitX], DoorCache[uid][dExitY], DoorCache[uid][dExitZ]);
	...
	else
	{
		...
		else DInfo(playerid, "Drzwi s¹ zamkniête.");
	}
	return 1;
}
	...
	if(sscanf(params, "s[16]", opcja))
	{
    	if(Player[playerid][Admin] >= 3)
	    {
			if(drzwi)
			{
	        	...
	        	"1.\tZmieñ nazwê systemow¹\n\
	        	2.\tZmieñ nazwê wyœwietlan¹\n\
	        	3.\tWyœwietl informacje\n\
	        	4.\tPrzypisz pod grupê\n\
				5.\tW³¹cz / wy³¹cz samoobs³ugê\n\
				6.\tW³¹cz / wy³¹cz przejazd\n\
				7.\tSchowek\n\
				8.\tKoszt wejœcia\n\
				9.\tUstaw w³aœciciela\n\
				10.\tZamra¿aj przy wejœciu\n\
				11.\tUstaw muzykê\n\
		...
		else
		{
			...
			else DInfo(playerid, "Nie znajdujesz siê w budynku, który mo¿esz edytowaæ.");
		}
	}
	else
	{
...
CMD:adoors(playerid, params[])
{
	...
	else if(!strcmp(typ, "stworz", true))
	{
		...
		{
			ShowPlayerDialog(playerid, DUMMY, DIALOG_STYLE_MSGBOX, "Tworzenie drzwi", "Znajdujesz siê aktualnie w kreatorze drzwi. Po zamkniêciu tego komunikatu, udaj siê do punktu, gdzie drzwi maj¹ prowadziæ.\nInteriory znajdziesz pod komend¹: {FF0000}/interiors{FFFFFF}.\nGdy bêdziesz ju¿ w miejscu wejœcia, wpisz: {FF0000}/adoors stworz{FFFFFF}.",
			"Ok", "Anuluj");
	...
	else if(!strcmp(typ, "usun", true))
	{
		...
		else
		{
			...
			DeleteDoors(uid);
			new string[128];
			format(string, 128, "Usun¹³eœ drzwi %s[%d]", DoorCache[index][dName], uid);
	...
	else if(!strcmp(typ, "pickup", true))
	{
		...
		else
		{
			...
			SaveDoors(uid);
			DInfo(playerid, "Zmieni³eœ pickupa drzwi.");
		}
	}
	else if(!strcmp(typ, "entervw", true))
	{
		...
		else
		{
			...
			SaveDoors(uid);
			DInfo(playerid, "Zmieni³eœ Virtual World wejœcia drzwi.");
		}
	}
	else if(!strcmp(typ, "exitvw", true))
	{
		...
		else
		{
			...
			SaveDoors(uid);
			DInfo(playerid, "Zmieni³eœ Virtual World wyjœcia drzwi.");
		}
	}
